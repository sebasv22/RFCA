 ...
 */
public class RandomTree_SQRT extends AbstractClassifier implements OptionHandler,
  ...
  public void buildClassifier(Instances data) throws Exception {
    ...
    // System.out.println("\n Entre a buildClassifier de RandomTree ");  
   ...
   /* if (m_KValue < 1) {
     ...
     if (m_KValue < 1) {
      m_KValue = (int) Math.sqrt((data.numAttributes() - 1));
    }
    ...
    if (data.classAttribute().isNumeric()) {
      trainVariance =
        RandomTree_SQRT.singleVariance(classProbs[0], totalSumSquared, totalWeight)
          / totalWeight;
  ...
  protected class Tree implements Serializable {
    ...
    protected void backfitData(Instances data, double[] classProbs,
      ...
      if (data.classAttribute().isNumeric()) {
        ...
        priorVar =
          RandomTree_SQRT.singleVariance(totalSum, totalSumSquared,
            totalSumOfWeights);
      }
      ...
      if (attribute.isNominal()) {
        ...
        for (int i = 0; i < indexOfFirstMissingValue; i++) {
          ...
          if (inst.value(att) > currSplit) {
            currVal =
              RandomTree_SQRT.variance(currSums, currSumSquared, currSumOfWeights);
            if (currVal < bestVal) {
              bestVal = currVal;
  ...
  public static void main(String[] argv) {
    runClassifier(new RandomTree_SQRT(), argv);
  }
}
